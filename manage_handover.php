...
<div class="container-fluid">
    <form action="" id="manage-handover">
        ...
        <div class="form-group">
            <label for="" class="control-label">Código de Referência da Solicitação</label>
            <input type="text" class="form-control" name="ref_code"  value="<?php echo isset($ref_code) ? $ref_code :'' ?>" required>
        </div>
        <div class="form-group">
            <button class="btn btn-sm btn-primary" type="button" id="chk_request">Verificar</button>
        </div>
        ...
        <div class="form-group" style="display: none">
            <label for="" class="control-label">Recebido por: </label>
...
<script>
            ...
            success:function(resp){
                if(resp){
                    ...
                    if(resp.status == 1){
                        ...
                        _html += '<p>Paciente: '+resp.data.id+'<b></b></p>';
                        _html += '<p>Grupo Sanguíneo: '+resp.data.blood_group+'<b></b></p>';
                        _html += '<p>Volume necessário: '+resp.data.volumeL+' L<b></b></p>';
                        _html += '<p>Médico(a): '+resp.data.physician_name+'<b></b></p>';
    ...
    $('#manage-handover').submit(function(e){
        ...
        if($('[name="volume"]').val() > $('[name="avolume"]').val()){
            $('#msg').html('<div class="alert alert-danger">O volume sanguíneo é maior que o volume disponível.</div>')
            end_load()
            return false;
        }
        $.ajax({
